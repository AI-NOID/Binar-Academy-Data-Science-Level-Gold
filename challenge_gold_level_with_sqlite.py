...
def upload_download_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            df = pd.read_csv(os.path.join(app.config['UPLOAD_FOLDER'], filename), encoding='latin-1')
            df = df.select_dtypes(include=['object']).applymap(cleansing)
            df = df.applymap(replaceThreeOrMore)
            df_clean = df.select_dtypes(include=['object']).applymap(remove_stopwords)
            df_clean = df_clean.to_csv(os.path.join(app.config['UPLOAD_FOLDER'], "data_clean.csv"), index=False, encoding='latin-1')

        # Convert the dataframe to an HTML table
        # Replace the existing table with the cleaned data
            conn = sqlite3.connect('database/challenge_level_3.db')
            cursor = conn.cursor()
            table = df.to_sql('upload_and_download_csv_file', conn, if_exists='replace') # to prove that this code is running well, drop the "challenge_cleaned_flask_swagger" table from the database via the app_sqlite.py file
            conn.close()
...
def upload_file_swgr_download():
    ...
    # Return the HTML table as the response to the form submission
        ...
        df_cleaned = table_2.to_csv("nama_file.csv", index=False)
